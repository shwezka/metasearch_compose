...
fun ProfileEdit(
    ...
    onNavigateToProf: ()-> Unit
        ...
        var date by remember { mutableStateOf(user.birthDate) }
        ...
        Surface(
            ...
            if(isLoading){
                ...
                Column(
                    ...
                    checkInput = isButtonEnabled && (user.name != nameInput || user.phone != phoneInput || user.birthDate != date || imageUri!= null)
                    AppButton(
                        lambda = {
                            openDialog = true
                        },
                    ...
                    if (openDialog) {
                        if (!checkInput) {
                            AlertDialog(
                                ...
                                title = { Text(text = "Вы не внесли изменения") },
                                text = { Text(text = "Данные, которые вы ввели, ничем не отличаются от существующих. Пожалуйста, измените введенные данные") },
                                confirmButton = {
                                    Button(
                                        onClick = {
                                            openDialog = false
                            ...
                            AlertDialog(
                                onDismissRequest = { openDialog = false },
                                title = { Text(text = "Данные успешно изменены") },
                                confirmButton = {
                                    Button(
                                        onClick = {
                                            openDialog = false
                                            isLoading = true
                                            user.updateUser(nameInput, phoneInput, date, imageUri.toString())
                                            updateUserData(user)
                                            isLoading = false
                                            onNavigateToProf()
                                        }
